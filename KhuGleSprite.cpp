...
CKhuGleSprite::CKhuGleSprite(int nType, int nCollisionType, CKgLine lnLine, KgColor24 fgColor, bool bFill, int nSliceOrWidth, int nLife)
{
	m_nType = nType;
	...
	if(m_nType == GP_STYPE_LINE|| m_nType == GP_STYPE_BLOCK)
		m_lnLine = lnLine;
	else 
...
void CKhuGleSprite::Render()
{
	...
	KgColor24 SaveColor = m_fgColor; // 변환되기 전 색깔

	if(m_bCollided)
		m_fgColor = KG_COLOR_24_RGB(255, 255, 0); // 충돌시 색깔 변환
...
//			m_lnLine.Start.X, m_lnLine.Start.Y, m_lnLine.End.X, m_lnLine.End.Y, KG_COLOR_24_RGB(255, 255, 255));
		...
		if (m_nType == GP_STYPE_BLOCK)
		{
			DrawLine(Parent->m_ImageR, Parent->m_ImageG, Parent->m_ImageB,
				Parent->m_nW, Parent->m_nH,
				(int)(m_lnLine.Start.X + Normal1.x), (int)(m_lnLine.Start.Y + Normal1.y), (int)(m_lnLine.Start.X + Normal2.x), (int)(m_lnLine.Start.Y + Normal2.y), m_fgColor);
			DrawLine(Parent->m_ImageR, Parent->m_ImageG, Parent->m_ImageB,
				Parent->m_nW, Parent->m_nH,
				(int)(m_lnLine.End.X + Normal1.x), (int)(m_lnLine.End.Y + Normal1.y), (int)(m_lnLine.End.X + Normal2.x), (int)(m_lnLine.End.Y + Normal2.y), m_fgColor);
		}
	}
	else if(!m_bFill)
	{
	...
	m_fgColor = SaveColor; //원레 색깔로 돌려주기
}
...
void CKhuGleSprite::AdjustColor()
{
	if (m_nLife == 1)
		m_fgColor = KG_COLOR_24_RGB(0, 255, 0);
	else if (m_nLife == 2)
		m_fgColor = KG_COLOR_24_RGB(0, 0, 255);
	else
	{
		m_fgColor = KG_COLOR_24_RGB(255, 0, 0);
	}
}
